...
class ChatHistoryManager:
    ...
    Args:
        user_id (str) = 사용자의 id
        room_id (str) = 사용자의 챗룸 id
        session_id (str) = Redis 내에서 사용자-챗룸 조합을 식별하기 위한 고유 키 (형식: "user_id:room_id")
    ...
    def __init__(self, user_id, room_id, model, max_token_limit=200):
        """ChatHistoryManager 클래스 생성자"""
        self.user_id = str(user_id)
        self.room_id = str(room_id)
        self.model = model
        self.max_token_limit = max_token_limit
        self.session_id = f"{self.user_id}:{self.room_id}"
